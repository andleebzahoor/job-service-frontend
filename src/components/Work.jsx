// // Updated Work component with Client form popup
...
//   // 🔥 First login and no role saved yet → show role selection
//   if (isFirstLogin === "true" && !storedRole) {
//     setRole("choose");
//     return; // ✅ STOP here — don't remove flag yet
//   }

//   // ✅ Role already selected → clear first_login flag
//   if (storedRole) {
//     localStorage.removeItem("first_login");
//   }
// }, []);

//   // ✅ Prevent wrong provider data showing if user switches account
//   // ✅ Redirect if provider already registered
//  useEffect(() => {
...
//   // ✅ If user is *provider* and already submitted form → redirect home
...
//     try {
//       const response = await fetch("http://127.0.0.1:5000/register_provider", {
//         method: "POST",
//         body: formData,
//       });
...
//   await fetch("http://127.0.0.1:5000/set_role", {
//     method: "POST",
...
//   if (role === "provider") {
//     // ✅ Open provider form modal — do NOT redirect
//     setRole("provider");
//   } else {
...
//         {/* ✅ Provider Button (unchanged logic) */}
//         <button
//           onClick={() => {
//             if (!user) {
...
//             const providerExists = localStorage.getItem(
//               `serviceProvider_${user.id}`
//             );

//             if (providerExists) {
//               alert("✅ You already submitted provider form!");
//               window.location.href = "/"; // redirect home or provider dashboard
//               return;
//             }
...
//           <div className="bg-white p-8 rounded-xl shadow-xl z-10 text-center animate-fadeIn">
...
//           <div
//             className={`bg-white w-full max-w-xl p-6 rounded-lg shadow-lg relative z-10 transform transition-all duration-300 ${showModal ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-8"
//               }`}
//           >
//             <button
//               onClick={() => setRole("")}
...
//                 className={`mt-2 px-6 py-3 rounded-lg shadow w-full text-white ${isFormValid ? "bg-green-600 hover:bg-green-700" : "bg-gray-400 cursor-not-allowed"
//                   }`}
//               >
//                 Submit
//               </button>
